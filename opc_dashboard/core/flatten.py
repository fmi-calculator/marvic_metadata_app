...
def flatten_models(record: dict) -> list[dict]:
    ...
    for i, m in enumerate(record.get("models", [])):
        rows.append({
            ...
            "scientific_publication_dois": _join(m.get("scientific_publication_dois", [])),
...
def flatten_crcf_matrix(record: dict) -> list[dict]:
    ...
    for code, label in _CRITERIA:
        ...
        row = {
            ...
            "min_accuracy_bias_lte_pmu":           cr.get("bias", "") if code == "minimum_accuracy" else "",
            "min_accuracy_pi_90pct":               cr.get("pi",   "") if code == "minimum_accuracy" else "",
            "min_accuracy_r2_gt_0":                cr.get("r2",   "") if code == "minimum_accuracy" else "",
        }
        rows.append(row)
    return rows
